...
#pragma HLS array_partition variable=A block (FACTOR / 4)=(FACTOR / 4) dim=2
#pragma HLS array_partition variable=B block (FACTOR / 4)=(FACTOR / 4) dim=2
...
#pragma HLS PIPELINE II=1
				...
				L1:for (int ia = 0; ia < A_COL_MAX; ++ia)
				{
					L2:for (int ib = 0; ib < B_COL; ++ib)
					{
						...
						L3:for(int ic = 0; ic < B_ROW_MAX/(FACTOR / 4); ++ic){
#pragma HLS PIPELINE II=1
							L4:for(int id = 0; id < (FACTOR / 4); ++id){
								sum += A[ia][id*B_ROW_MAX/(FACTOR / 4)+ic] * B[ib][id*B_ROW_MAX/(FACTOR / 4)+ic];
							}
						}
